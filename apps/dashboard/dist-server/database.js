...
export async function insertRefreshToken(tokenHash, username, expiresAt) {
    await getPool().query(`INSERT INTO refresh_tokens (token_hash, username, expires_at)
     VALUES ($1, $2, $3)
     ON CONFLICT (token_hash) DO NOTHING`, [tokenHash, username, expiresAt]);
}
...
export async function touchRefreshToken(tokenHash) {
    await getPool().query("UPDATE refresh_tokens SET last_used_at = now() WHERE token_hash = $1", [tokenHash]);
}
export async function revokeRefreshToken(tokenHash) {
    await getPool().query("UPDATE refresh_tokens SET revoked_at = now() WHERE token_hash = $1 AND revoked_at IS NULL", [tokenHash]);
}
export async function revokeAllRefreshTokens(username) {
    await getPool().query("UPDATE refresh_tokens SET revoked_at = now() WHERE username = $1 AND revoked_at IS NULL", [username]);
}
