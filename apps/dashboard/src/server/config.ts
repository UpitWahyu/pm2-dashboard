...
function required(name: string, minLen: number): string | null {
...
function parseEnvServers(): ServerConfig[] {
  ...
  try {
    ...
    return arr.map((s) => {
      const item = s as { name?: string; url?: string; token?: string; port?: number };
      return {
        id: -1,
        name: item.name ?? "",
...
export async function loadActiveServers(): Promise<{ servers: ServerConfig[]; dbAvailable: boolean }> {
  if (await db.dbAvailable()) {
    try {
      ...
      console.error(`[config] gagal baca server dari DB, pakai .env: ${(err as Error).message}`);
    }
  }
